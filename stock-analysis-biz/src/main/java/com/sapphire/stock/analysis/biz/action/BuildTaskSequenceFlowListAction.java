...
public class BuildTaskSequenceFlowListAction implements BusinessAction {
    ...
    public void process(ProcessContext processContext) {
        ...
        long scheduleJobId = entity.getFlinkScheduleJob().getId();
        List<FlinkSQLJob> flinkSQLJobs = flinkSqlJobRepository.selectByScheduleJobId(scheduleJobId);

        //添加父节点关联的子节点
        for (FlinkSQLJob job : flinkSQLJobs) {
            ...
            taskInfo.put("FLINK_SQL_JOB_ID", String.valueOf(job.getId()));
