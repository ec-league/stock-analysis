...
export default class SceneList extends PureComponent {
  ...
  fetch = (params = {}) => {
    ...
    dispatch({
      type: 'scene/fetchSceneListEffect',
      payload: {
        ...params,
      },
    })
  ...
  renderSceneList() {
    ...
    //获取当前列表和分页数据
    const { scene_list: sceneList, pagination } = scene;
    ...
    const columns = [
      ...
      },
      {
        ...
        render: (text, record) => {
          return (
            <span>
            <a href={`/model-manage/scene/scene-detail/${record.id}`}>详情</a>
          </span>
          );
        },
      }
