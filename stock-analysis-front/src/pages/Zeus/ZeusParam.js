...
export default class ZeusParam extends PureComponent {
  ...
  fetch = (params = {}) => {
    ...
    dispatch({
      type: 'zeusparam/fetchSceneListEffect',
      payload: {
        ...params,
      },
    })
  ...
  renderZeusParam() {
    ...
    //获取Model state
    const { zeusparam } = this.props;

    //获取当前列表和分页数据
    const { scene_list: sceneList, pagination } = zeusparam;
  ...
  render() {
    return (
      <PageHeaderWrapper>
        <Card title="参数列表" bordered={true}>
          {this.renderZeusParam()}
        </Card>
      </PageHeaderWrapper>
    );
  }
