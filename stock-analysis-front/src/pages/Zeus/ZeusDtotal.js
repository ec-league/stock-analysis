...
export default class ZeusDtotal extends PureComponent {
  ...
  fetch = (params = {}) => {
    ...
    dispatch({
      type: 'zeusdtotal/fetchSceneListEffect',
      payload: {
        ...params,
      },
    })
  ...
  renderZeusDtotal() {
    ...
    //获取Model state
    const { zeusdtotal } = this.props;

    //获取当前列表和分页数据
    const { scene_list: sceneList, pagination } = zeusdtotal;
  ...
  render() {
    return (
      <PageHeaderWrapper>
        <Card title="决策总数列表" bordered={true}>
          {this.renderZeusDtotal()}
        </Card>
      </PageHeaderWrapper>
    );
  }
