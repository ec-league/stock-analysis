...
export default class ZeusDreason extends PureComponent {
  ...
  fetch = (params = {}) => {
    ...
    dispatch({
      type: 'zeusdreason/fetchSceneListEffect',
      payload: {
        ...params,
      },
    })
  ...
  renderZeusDreason() {
    ...
    //获取Model state
    const { zeusdreason } = this.props;

    //获取当前列表和分页数据
    const { scene_list: sceneList, pagination } = zeusdreason;
  ...
  render() {
    return (
      <PageHeaderWrapper>
        <Card title="决策理由列表" bordered={true}>
          {this.renderZeusDreason()}
        </Card>
      </PageHeaderWrapper>
    );
  }
